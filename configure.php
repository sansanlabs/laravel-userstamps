  ...
  return preg_split(
    ...
    run(
      "dir /S /B * | findstr /v /i .git\ | findstr /v /i vendor | findstr /v /i " .
        basename(__FILE__) .
        ' | findstr /r /i /M /F:/ ":author :vendor :package VendorName skeleton migration_table_name vendor_name vendor_slug author@domain.com"',
    ),
  );
}
...
function replaceForAllOtherOSes(): array {
  return explode(
    PHP_EOL,
    run(
      'grep -E -r -l -i ":author|:vendor|:package|VendorName|skeleton|migration_table_name|vendor_name|vendor_slug|author@domain.com" --exclude-dir=vendor ./* ./.github/* | grep -v ' .
        basename(__FILE__),
    ),
  );
}
...
function getGitHubApiEndpoint(string $endpoint): ?stdClass {
  try {
    ...
    curl_setopt_array($curl, [
      ...
      CURLOPT_HTTPHEADER => ["User-Agent: spatie-configure-script/1.0"],
    ]);
...
function searchCommitsForGitHubUsername(): string {
  ...
  $committers = array_filter(
    array_map(function ($line) use ($authorName) {
      $line = trim($line);
...
foreach ($files as $file) {
  ...
  match (true) {
    ...
    str_contains($file, determineSeparator("src/SkeletonServiceProvider.php")) => rename(
      $file,
      determineSeparator("./src/" . $className . "ServiceProvider.php"),
    ),
    str_contains($file, determineSeparator("src/Facades/Skeleton.php")) => rename($file, determineSeparator("./src/Facades/" . $className . ".php")),
    str_contains($file, determineSeparator("src/Commands/SkeletonCommand.php")) => rename(
      $file,
      determineSeparator("./src/Commands/" . $className . "Command.php"),
    ),
    str_contains($file, determineSeparator("database/migrations/create_skeleton_table.php.stub")) => rename(
      $file,
      determineSeparator("./database/migrations/create_" . title_snake($packageSlugWithoutPrefix) . "_table.php.stub"),
    ),
    str_contains($file, determineSeparator("config/skeleton.php")) => rename(
      $file,
      determineSeparator("./config/" . $packageSlugWithoutPrefix . ".php"),
    ),
...
if (!$usePhpStan) {
  ...
  remove_composer_deps(["phpstan/extension-installer", "phpstan/phpstan-deprecation-rules", "phpstan/phpstan-phpunit", "larastan/larastan"]);
